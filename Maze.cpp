...
// =======================> is_open <===============================
int Maze::is open(int i, int j) of this maze is open (contains a '.')
{
	if (i < 1 || i > _max_row) 
...

{
		...
		if (_map[row][col] != '*')
			_map[row][col] = '.';
// reset current spot in maze to passible

	return;
}

// ====================> stream output <=========================
ostream& operator<<(ostream& steeam, Maze m)
{
...
{	char ch;
	...
	for(int row = 1; row <= m.max-row; row++)
	{
		for(int col = 1; col <= m.max-col; col++)
			{
				stream.get(ch);
				if (ch == '.' || ch == '')
					m._map[row][col] = '.';
